...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
        ...
        self.learning_rate_set_op = self.learning_rate.assign(self.learning_rate * 10)
        ...
        def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
          return rl_seq2seq.embedding_rnn_seq2seq(
              encoder_inputs,
              decoder_inputs,
              cell,
        ...
        self.gradient_norms = []
        self.updates = []
        self.advantage = [tf.placeholder(tf.float32, shape=[None], name="advantage_%i" % i) for i in range(len(buckets))]
        opt = tf.train.GradientDescentOptimizer(self.learning_rate)
        #opt = tf.train.AdamOptimizer()
        for b in xrange(len(buckets)):
            self.adjusted_losses =tf.reduce_sum( tf.matmul(tf.reshape(self.losses[b],[1,-1]), tf.reshape(self.advantage[b],[-1,1])))
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only=True, force_dec_input=False, advantage=None, debug=False):
    ...
    for l in xrange(len(self.buckets)):
      input_feed[self.advantage[l].name] = advantage[l] if advantage else defaltvec
  ...
  def step_rl(self, session, buckets, encoder_inputs, decoder_inputs, target_weights, batch_source_encoder,batch_source_decoder, bucket_id, rev_vocab=None, debug=False,disSession=None,disModel=None):
    ...
    while True:
      ...
      '''
      resp_c = self.ids2tokens(resp_tokens,rev_vocab)
      for ii in range(len(resp_c)):
        cc=""
        for ww in resp_c[ii]:
          cc=cc+" "+ww
        print(cc)
      ...
      #----------------------------------------------------
      '''
      X1 = np.zeros((disModel.config['batch_size']*2, disModel.config['data1_maxlen']), dtype=np.int32)
      X1_len = np.zeros((disModel.config['batch_size']*2,), dtype=np.int32)
      X2 = np.zeros((disModel.config['batch_size']*2, disModel.config['data2_maxlen']), dtype=np.int32)
      X2_len = np.zeros((disModel.config['batch_size']*2,), dtype=np.int32)
      Y = np.zeros((disModel.config['batch_size']*2,), dtype=np.int32)

      Y[::2] = 1
      X1[:] = disModel.config['fill_word']
      X2[:] = disModel.config['fill_word']
      for i in range(disModel.config['batch_size']):
        d1_len = min(disModel.config['data1_maxlen'], len(batch_source_encoder[i]))
        d2p_len = min(disModel.config['data2_maxlen'], len(resp_tokens[i]))
        d2n_len = min(disModel.config['data2_maxlen'], len(resp_tokens[i]))
        X1[i*2,   :d1_len],  X1_len[i*2]   = batch_source_encoder[i][:d1_len],   d1_len
        X2[i*2,   :d2p_len], X2_len[i*2]   = resp_tokens[i][:d2p_len], d2p_len
        X1[i*2+1, :d1_len],  X1_len[i*2+1] = batch_source_encoder[i][:d1_len],   d1_len
        X2[i*2+1, :d2n_len], X2_len[i*2+1] = resp_tokens[i][:d2n_len], d2n_len
      feed_dict={ disModel.X1: X1, disModel.X1_len: X1_len, disModel.X2: X2,
                        disModel.X2_len: X2_len, disModel.Y: Y}
      pred,loss = disModel.test_step(disSession, feed_dict)
      r4=[]
      mmm=0
      for ll in pred:
        if mmm%2==0:
          r4.append(-1*float(ll))
        mmm=mmm+1
      #r4 = 1/(1+np.exp(-r4))
      #print("r4: %s" %(r4))
      R = r4
    ...
    for ii in range(self.batch_size):
      defaltvec.append(2)
    if len(ep_rewards) == 0: ep_rewards.append(defaltvec)
    #print("[Step] final:", episode, ep_rewards)
    # gradient decent according to batch rewards
    advantage = ep_rewards * len(buckets)
    #print("advantage: %s" %advantage)

    _, step_loss, _ = self.step(session, init_inputs[0], init_inputs[1], init_inputs[2], init_inputs[3],
              forward_only=False, force_dec_input=True, advantage=advantage)
    ansloss=[]
    for ii in range(len(step_loss)):
      ansloss.append(step_loss[ii]*advantage[bucket_id][ii])
    return None, ansloss, None
...
# log(P(b|a)), the conditional likelyhood
  def logProb(self, session, buckets, tokens_a, tokens_b, mask=None):
    ...
    for i, (tokens, logits) in enumerate(zip(tokens_b, logits_t)):
        ...
        #print ("logProb: p: %s" %(p))
        sum_p.append(p)
    re = sum_p#np.sum(sum_p)/len(sum_p)
  ...
  def ids2tokens(self, ids, rev_vocab):
     txt=[]
     for sen in ids:
       sss=[]
       for t in sen:
         sss.append(rev_vocab[int(t)])
       txt.append(sss)
     return txt
  ...
  def get_batch_dev(self, train_data, bucket_id, type=0, fixed_set=None, weibo_set=None, qa_set=None):
    ...
    batch_source_encoder, batch_source_decoder = [], []
    #print("bucket_id: %s" %bucket_id)
    leng=len(train_data[bucket_id])
    for batch_i in xrange(self.batch_size):
      encoder_input, decoder_input = train_data[bucket_id][(type+batch_i)%leng]
          #print("train en: %s, de: %s" %(encoder_input, decoder_input))
    ...
    for length_idx in xrange(decoder_size):
      ...
      batch_weights.append(batch_weight)
    return batch_encoder_inputs, batch_decoder_inputs, batch_weights, batch_source_encoder, batch_source_decoder
  def caldummy(self, session, buckets, encoder_inputs, decoder_inputs, target_weights, batch_source_encoder,batch_source_decoder, bucket_id, rev_vocab=None):
    batch_mask = [1 for _ in xrange(self.batch_size)]
    r1=[]
    for d in self.dummy_dialogs:
      pp = self.logProb(session, buckets, batch_source_encoder, [d for _ in batch_source_encoder], mask= batch_mask)
      r1.append(pp)
    r1 = -np.mean(r1) if len(r1)>0 else 0
    #print("r1: final value: ", r1)
    return r1
  def calprob(self, session, buckets, encoder_inputs, decoder_inputs, target_weights, batch_source_encoder,batch_source_decoder, bucket_id, rev_vocab=None):
    encoder_state, step_loss, output_logits = self.step(session, encoder_inputs, decoder_inputs, target_weights,bucket_id, forward_only=True, force_dec_input=False)
    resp_tokens = self.remove_type(output_logits, self.buckets[bucket_id], type=1)
    batch_mask = [1 for _ in xrange(self.batch_size)]
    r3 = self.logProb(session, buckets, resp_tokens, batch_source_encoder, mask= batch_mask)
    #print(r3)
    return r3
