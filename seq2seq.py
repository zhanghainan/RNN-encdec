...
embed_dict = pt.io.base.read_embedding(filename='./OpenSubData/embedding.txt')
embed_dict[0] = np.zeros((300, ), dtype=np.float32)
W_init_embed = np.float32(np.random.uniform(-0.02, 0.02, [5000, 300]))
init_embedding = pt.io.base.convert_embed_2_numpy(embed_dict, embed = W_init_embed)
...
def embedding_rnn_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_decoder") as scope:
    ...
    embedding = variable_scope.get_variable("embedding",initializer = init_embedding)
...
def embedding_tied_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    embedding = variable_scope.get_variable(
        "embedding", dtype=dtype,initializer=init_embedding)
...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      x = linear([inp] + initial_state, input_size, True)
      # Run the RNN.
